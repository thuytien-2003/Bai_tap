    ...
    async function fetchWeather() {
      try {
        const currentRes = await fetch(
          `https://api.weatherapi.com/v1/current.json?key=${API_KEY}&q=${encodeURIComponent(city)}&aqi=no&lang=vi`
        );
        ...
        const forecastRes = await fetch(
          `https://api.weatherapi.com/v1/forecast.json?key=${API_KEY}&q=${encodeURIComponent(city)}&days=1&aqi=no&alerts=no&lang=vi`
        );
          ...
          <div className={styles.currentWeather}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <div>
                ...
                <div className={styles.conditionText}>{weather.current.condition.text}</div>
              </div>
              <img src={weather.current.condition.icon} alt="icon" style={{ width: 64, height: 64 }} />
            </div>
            ...
            <div className={styles.hourlyWrapper}>
              <div className={styles.hourly}>
                {forecast.map((hour, index) => {
                  const isNow = new Date(hour.time).getHours() === new Date(localTime).getHours();
                  return (
